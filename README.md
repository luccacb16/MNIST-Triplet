...
<img src="https://latex.codecogs.com/svg.image?accuracy=\frac{\sum_{i=1}^{n}(distance_i<threshold)==label_i}{n}" title="accuracy=\frac{\sum_{i=1}^{n}(distance_i<threshold)==label_i}{n}" />
Sendo a distância calculada utilizando a norma L2 entre os embeddings dos pares.

- **VAL (Validation Rate)**: 
<img src="https://latex.codecogs.com/svg.image?VAL=\frac{TP}{TP&plus;FN}" title="VAL=\frac{TP}{TP+FN}" />
