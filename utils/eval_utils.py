...
def get_pairs(ids_df: pd.DataFrame, n_pairs: int = None) -> pd.DataFrame:
    if n_pairs is None:
        n_pairs = len(ids_df)
    ...
    while i < n_pairs // 2:
        ...
        same_id_df = ids_df[(ids_df['label'] == row['label']) & ~(ids_df.index == row.name)]
        if same_id_df.empty:
            continue
        
        same = same_id_df.sample(1)
        diff = ids_df[ids_df['label'] != row['label']].sample(1)

        img1_array = row[1:].values.reshape(28, 28).astype(np.uint8)
        img2_array_same = same.iloc[0, 1:].values.reshape(28, 28).astype(np.uint8)
        img2_array_diff = diff.iloc[0, 1:].values.reshape(28, 28).astype(np.uint8)
        
        pairs.append([img1_array, img2_array_same, 1])
        pairs.append([img1_array, img2_array_diff, 0])
...
def eval_epochs(epochs_path: str, pairs: pd.DataFrame, model_class: nn.Module, batch_size: int = 32, transform = None, device: str = 'cuda', target_far=1e-2) -> None:
    ...
    for model in models_name_sorted:        
        ...
        far = FAR(pairs_dist, low_far)
        
        print(f'Target FAR: {target_far:.0e} | Threshold: {low_far:.4f}')
        print(f'[{model}] Accuracy: {acc:.4f}')
        print(f'[{model}] VAL: {val:.4f} | FAR: {far:.4f}')
